...
const Tcard = ({
  ...
  return (
    <div className="bg-gray-950/40 rounded-2xl p-3 mb-2 text-white border border-gray-700">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs">
              {copiers.toLocaleString()} Copiers
            </p>
          </div>
        </div>
        <button className="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-md text-xs font-medium transition-colors">
          Copy
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          <p className="text-gray-400 mb-0.5">Return</p>
          <p className="text-green-400 font-semibold">+{returnPercent}%</p>
        </div>
        <div>
          <p className="text-gray-400 mb-0.5">Risk</p>
          <p className="text-gray-400 font-semibold">Score</p>
        </div>
        <div>
          <p className="text-gray-400 mb-0.5">Return</p>
          <p className="text-gray-400 font-semibold">{riskScore}</p>
        </div>
        <div>
...
const page = () => {
  return (
    <div className="min-h-screen bg-black grid p-2 grid-cols-12 items-center justify-start ">
      ...
      <div className="text-left max-w-4xl">
        < h1 className="text-6xl md:text-8xl font-light leading-tight">
          Building the Premier Copy Trading Hub
        </h1>
        ...
        <div className="max-w-164 w-full mx-auto space-y-1">
          {traders.map((trader, index) => (
            <Tcard
              key={index}
              emoji={trader.emoji}
              hashcode={trader.hashcode}
              copiers={trader.copiers}
              returnPercent={trader.returnPercent}
